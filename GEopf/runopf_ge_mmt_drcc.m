function [solution, information] = runopf_ge_mmt_drcc(mpc,mu,Sigma,eps)
...
Qptgmin = zeros(nPTG,1); Qptgmax = mpc.ptg(:,5);
...
for i = 1:n_nonowf
    ii = i_nonowf(i);
    a_i = alpha_Pg(ii) * ones(n_xi,1);
    cons_nonowf = [
        cons_nonowf;
        Pgmin(ii) + sqrt((1-eps)/eps) * norm(sigma_sqrt*a_i) <= Pg(ii) <= Pgmax(ii) - sqrt((1-eps)/eps) * norm(sigma_sqrt*a_i);
        ];
end
% 1.2) owf
cons_owf = [];
for i = 1:n_owf
    ...
    a_i = alpha_Pg(ii) * ones(n_xi,1) - e_i;
    cons_owf = [
        cons_owf;
        Pgmin(ii) + sqrt((1-eps)/eps) * norm(sigma_sqrt*a_i) <= Pg(ii) <= Pgmax(ii) - sqrt((1-eps)/eps) * norm(sigma_sqrt*a_i);
        ];
...
cons_Prs = [];
for i = 1:nGb
    a_i = alpha_Prs(i) * ones(n_xi,1);
    cons_Prs = [
        cons_Prs;
        Prsmin(i) + sqrt((1-eps)/eps) * norm(sigma_sqrt*a_i) <= Prs(i) <= Prsmin(i) - sqrt((1-eps)/eps) * norm(sigma_sqrt*a_i);
        ];
end
% 1.4) PGs
cons_PGs = [];
for i = 1:nGs
    a_i = alpha_PGs(i) * ones(n_xi,1);
    cons_PGs = [
        cons_PGs;
        PGsmin(i) + sqrt((1-eps)/eps) * norm(sigma_sqrt*a_i) <= PGs(i) <= PGsmin(i) - sqrt((1-eps)/eps) * norm(sigma_sqrt*a_i);
        ];
end
% 1.5) ptg
cons_Qptg = [];
for i = 1:nGs
    a_i = alpha_Qptg(i) * ones(n_xi,1);
    cons_Qptg = [
        cons_Qptg;
        Qptgmin(i) + sqrt((1-eps)/eps) * norm(sigma_sqrt*a_i) <= Qptg(i) <= Qptgmin(i) - sqrt((1-eps)/eps) * norm(sigma_sqrt*a_i);
        ];
end
boxCons_drcc = [
    cons_nonowf;
...
for i = 1:nl
    a_i = alpha_Pf(i) * ones(n_xi,1);
    electricityBranchFlowConsDC_drcc = [
        electricityBranchFlowConsDC_drcc;
        - upf(i) + sqrt((1-eps)/eps) * norm(sigma_sqrt*a_i) <= Pf(i) <= upf(i) - sqrt((1-eps)/eps) * norm(sigma_sqrt*a_i);
        ];
end
...
alpha_Qgpp = value(alpha_Qgpp);
alpha_Pf = value(alpha_Pf);
