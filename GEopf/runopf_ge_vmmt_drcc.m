function [solution, information] = runopf_ge_vmmt_drcc(mpc,mu,Sigma,xi_hat,eps)
...
delta_mu = 1 / sqrt(n_s) *1;   % n_s = 样本数量
delta_Sigma = 0.05 * norm(Sigma, 2)*1;

Sigma_tilde = Sigma + delta_Sigma * eye(size(Sigma));
Sigma_tilde = (Sigma_tilde + Sigma_tilde')/2;    % 强制对称
L = sqrtm(Sigma_tilde);
% L = chol(Sigma_tilde, 'lower');

% 1) box cons
cons_nonowf = [Pg >= Pgmin];
for i = 1:n_nonowf
    ii = i_nonowf(i);
    a_i = alpha_Pg(ii,:)* ones(n_xi,1);   % 现在是列向量
    b_i = Pgmax(ii) - Pg(ii);                  % 上界剩余量
    cons_nonowf = [cons_nonowf;
        delta_mu * norm(a_i,2) + sqrt((1-eps)/eps) * norm(L' * a_i,2) <= b_i
    ];
end
