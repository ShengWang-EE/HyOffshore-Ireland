...
% this function is to run optimal energy flow for hydrogen-blended gas and
% power systems using a deterministic optimization framework
%% grab some dimenssions and parameters
...
% natural gas, hydrogen properties
nGasType = 2;
[GCV, M, fs, a, R, T_stp, Prs_stp, Z_ref, T_gas, eta, CDF,rho_stp] = initializeParameters_J13();
%% set some parameters for optimization
% secuiity limit (including flame speed (FS) and Wobbe index (WI)) for
% contigent, normal, and further relax cases. values are set according to
% the gas regulations in the UK
FSlimit = 0.2; 
...
%% initial value as reference point for linearization
...
%% start sequential programming procedure
solverTime = zeros(iterationMax,1);
for v = 1:iterationMax
    yalmip('clear')
%% state variables
Prs_square      = sdpvar(nGb,1);                                            % square of nodal gas pressure, bar^2
PGs             = sdpvar(nGs,1);                                            % gas supply from gas sources, Mm3/day
Qd              = sdpvar(nGasLoad,nGasType);                                % nodal gas demand for each gas composition, Mm3/day
Qptg            = sdpvar(nPTG,2);                                           % gas production of PTG, [ methane; hydrogen ] Mm3/day
Pptg            = sdpvar(nPTG,1);                                           % electricity consumption of PTG, *100 MW
Pg              = sdpvar(size(mpc.gen,1),1);                                % include TPP, GPP and renewable generators, 1/100 MW
Qgpp            = sdpvar(nGpp, nGasType);                                   % gas consumption for GPPs, Mm3/day
Va              = sdpvar(nb,1);                                             % phase angle of voltage
gasComposition  = sdpvar(nGb,nGasType);                                     % gas composition
gasFlow         = sdpvar(nGl,nGasType);                                     % gas flow, Mm3/day            
sigma_PHI       = sdpvar(nGl,1);                                            % slack variable limit for gas flow during Taylor approximation
sigma_x         = sdpvar(nGb, nGasType);                                    % slack variable limit for gas composition
sigma_Qd        = sdpvar(nGasLoad, nGasType);                               % slack variable limit for gas demand
sigma_Qgpp      = sdpvar(nGpp,nGasType);                                    % slack variable limit for Qgpp
sigma_Gf        = sdpvar(nGl,nGasType);                                     % slack variable limit for gas flow
gamma           = mpc.Gline(:,9);                                           % gas flow direction
%
LCg = 0;                                                                    % gas curtailment is not allowed in this case
%% upper and lower bounds
Prsmin = mpc.Gbus(:,5); Prsmax = mpc.Gbus(:,6);                             % gas pressure bounds
PGsmin = mpc.Gsou(:,3); PGsmax = mpc.Gsou(:,4);                             % gas supply bounds
Qptgmin = mpc.ptg(:,4); Qptgmax = mpc.ptg(:,5);                             % limits of gas production from PTGs
QptgMax_hydrogen = Qptgmax;                                                 % limits of equivalent hydrogen production from PTGs
Pgmin = mpc.gen(:, PMIN) / baseMVA *0;                                      % bounds of generators
Pgmax = mpc.gen(:, PMAX) / baseMVA;
WImin = WI_ng/1e6 * (1-WIlimit); WImax = WI_ng/1e6 * (1+WIlimit);           % limtis of WI
FSmin = FS_ng * (1-FSlimit); FSmax = FS_ng * (1+FSlimit);                   % limits of FS
...
Val(refs) = 1;
gasFlowMax = mpc.Gline(:,5);                                                % gas flow limits
% pre-calculation:calculate the property factor (as in Weymouth equation) of each pipeline
...
electricityCons = [
    ...
    Pgmin <= Pg <= Pgmax;
    Va(refs) == 0;                                                          % reference at the slack bus
    ]:'electricityCons';
...
GCV_nodal = gasComposition * GCVall';                                       % gross carloric value
S_nodal = gasComposition * Mall' / M.air;                                   % specific gravity
sqrtS = 0.5 * (S_nodal/sqrt(S_ng) + sqrt(S_ng));                            % linear form of square root of specific gravity
...
% -------------------------------------------
% gas composition Taylor expansion
PTGbus = mpc.ptg(:,1); 
CgsPTG = sparse(PTGbus, (1:nPTG)', 1, nGb, nPTG); % connection matrix
...
gasCompositionCons1 = [];
% nodal gas injection of each gas composition
for r = 1:nGasType
