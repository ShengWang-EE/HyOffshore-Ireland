...
% mpc.gen(131:139,[2:6,9]) = mpc0.gen(131:139,[2:6,9]) ./ sum(mpc0.gen(131:139,9)) * 20000; % 20 GW
% [result1,info1] = runopf_ge(mpc);
% [HGEresults1, HGEinfo1] = runopf_hge(mpc,result1);
% % ------------------------------------------------------------------------
...
% mid term scenario, 20 GW, for validating uncertainty handling
...
for k = 1:24
    ...
    % gas demand 天然气增长的慢点
    mpcGE1{counter}.Gbus(:,3) = mpc0.Gbus(:,3) * gasDemandFactor(timeofday) * 1.01^(2030-2023);
    mpcGE2{counter}.Gbus(:,3) = mpc0.Gbus(:,3) * gasDemandFactor(timeofday) * 1.01^(2040-2023);
    mpcGE3{counter}.Gbus(:,3) = mpc0.Gbus(:,3) * gasDemandFactor(timeofday) * 1.01^(2050-2023);
    % update transmission factor
    mpcGE1{counter}.Gline(:,3) = mpc0.Gline(:,3) * 1.01^(2030-2023);
    mpcGE2{counter}.Gline(:,3) = mpc0.Gline(:,3) * 1.01^(2040-2023);
    mpcGE3{counter}.Gline(:,3) = mpc0.Gline(:,3) * 1.01^(2050-2023);
    % update the gas source
    mpcGE1{counter}.Gsou(:,4) = mpc0.Gsou(:,4) * 1.01^(2030-2023);
    mpcGE1{counter}.Gsou(:,4) = mpc0.Gsou(:,4) * 1.01^(2040-2023);
    mpcGE1{counter}.Gsou(:,4) = mpc0.Gsou(:,4) * 1.01^(2050-2023);
end
% drcc para
epsilon = 0.05;
...
for k = 1:24
    ...
    timeofday = mod(k-1,24)+1;
    % short term
    [HGEresults_drcc1{counter}, HGEinfo_drcc1{counter}] = runopf_hge_drcc(mpcGE1{counter},mu1(timeofday),sigma1(timeofday),epsilon);
    % mid term
    [HGEresults_drcc2{counter}, HGEinfo_drcc2{counter}] = runopf_hge_drcc(mpcGE2{counter},mu2(timeofday),sigma2(timeofday),epsilon);
    % long term
    [HGEresults_drcc3{counter}, HGEinfo_drcc3{counter}] = runopf_hge_drcc(mpcGE3{counter},mu3(timeofday),sigma3(timeofday),epsilon);
...
for k = 1:24
    ...
    timeofday = mod(k-1,24)+1;
    % short term
    [HGEresults_drcc1_new{counter}, HGEinfo_drcc1{counter}] = runopf_hge_drcc(mpcHGE1{counter},mu1(timeofday),sigma1(timeofday),epsilon);
    % mid term
    [HGEresults_drcc2_new{counter}, HGEinfo_drcc2{counter}] = runopf_hge_drcc(mpcHGE2{counter},mu2(timeofday),sigma2(timeofday),epsilon);
    % long term
    [HGEresults_drcc3_new{counter}, HGEinfo_drcc3{counter}] = runopf_hge_drcc(mpcHGE3{counter},mu3(timeofday),sigma3(timeofday),epsilon);
    ob.HGE(k,1:3) = [HGEinfo_drcc1{counter}.problem, HGEinfo_drcc2{counter}.problem, HGEinfo_drcc3{counter}.problem];
end
